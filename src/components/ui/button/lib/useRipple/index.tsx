...
/**
 * index - Material UI style ripple effect React hook
...
/**
 * HOF index - Generate a custom ripple hook with predefined options
 *
 * After generating a HOF index you can then override some or all predefined options by passing a new option object.
 * @author Jonathan Asplund <jonathan@asplund.net>
 * @param inputOptions ripple options
 * @returns Custom HOC index hook
 */
...
function createRipple<T extends HTMLElement>(
  ref: T,
  event: MinimalEvent,
  { className, color, duration, timingFunction }: Omit<Options, 'ref'>,
  ctx = document
): HTMLDivElement {
...
function cancelRippleAnimation<T extends HTMLElement>(
  element: T,
  options: Omit<Options<T>, 'cancelAutomatically' | 'color' | 'onSpawn' | 'ref'>
) {
